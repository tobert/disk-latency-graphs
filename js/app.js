 ...
 *
 * Modified from effio to make it work better on a static site with
 * a reduced data set.
 * Original: https://github.com/tobert/effio/blob/master/public/js/app.js
 */
...
APP.chart = function (target, devices, chart1, chart2) {
  var mid = d3.selectAll("#top_left");
      mid.selectAll("div").remove();
  var txt = mid.append("div").append("h3");
  ...
  // should be easy to add nvd3/whatever down the road ...
  if (ctype[0] === "c3") {
  ...
  // doesn't make sense to do two dimensions on a box chart (for now)
    ...
    txt.text("Try selecting HDD or SSD when using d3.box. (over there --->)");
...
APP.summaries_to_c3 = function (data, sample_type, fun) {
  return data.map(function (summary) {
    ...
    if (sample_type === "percentiles_bin") {
      ...
      col = summary[sample_type].map(fun).map(function (d) { return Math.round(d); });
    }
...
APP.d3box = function (target, summaries, sample_type, fun) {
  ...
  var svg = d3.select("#mid_left").selectAll("svg")
    .data(data)
    .enter().append("svg")
...
// set up regions on the screen using bootstrap
...
APP.setup_chart_controls = function (target) {
  ...
  top_div.append("div").classed({"col-xs-8": true}).attr("id", "top_left");
  top_div.append("div").classed({"col-xs-2": true}).attr("id", "top_middle");
  top_div.append("div").classed({"col-xs-2": true}).attr("id", "top_right");
  mid_div.append("div").classed({"col-xs-12": true, "col-md-8": true}).attr("id", "mid_left");
  mid_div.append("div").classed({"col-xs-6": true, "col-md-2": true}).attr("id", "mid_middle");
  mid_div.append("div").classed({"col-xs-6": true, "col-md-2": true}).attr("id", "mid_right");
  bot_div.append("div").classed({"col-xs-12": true, "col-md-12": true}).attr("id", "bot_left");
};
...
APP.change = function (side) {
  ...
  var chart2 = {name: "chart2"}; // disabled for now
  ...
  APP.chart("#mid_left", devs, chart1, chart2);
};
...
APP.build_nav = function() {
  ...
  var mid_middle = d3.select("#mid_middle");
  mid_middle.append("div").append("strong").text("Benchmark");
  var benchmarks = mid_middle.selectAll(".chart1-benchmark-radio")
  ...
  mid_right.append("div").append("strong").text("Media Type");
  ...
  mid_right.append("div").append("strong").text("Fio Log");
  var logtypes = mid_right.selectAll(".chart1-logtype-radio")
    .data([["Latency", "lat"], ["Bandwidth", "bw"], ["IOPS", "iops"]])
    .enter()
    .append("div")
  ...
  mid_right.append("div").append("strong").text("Samples");
  var pcntls = mid_right.selectAll(".pcntl-radio")
    .data([["All", "all"], ["<=P1", "p1"], [">=P99", "p99"]])
    .enter()
    .append("div")
  ...
  mid_right.append("div").append("strong").text("Chart Type");
  ...
  mid_right.append("div").append("strong").text("Data Direction");
  ...
  mid_right.append("div").append("strong").text("Aggregate Field");
  ...
  bot_left.append("div").append("strong").text("Devices to Display:");
  var devs = bot_left.selectAll(".device-checkbox")
...
// A simpler version of run() that downloads a single pre-cooked
// json file. Used to post a dataset publicly.
// var prms = APP.cooked("/data/f37a1233dd0d2a78e386bd5fcf1a5377d33ba58e.json");
APP.cooked = function (url) {
  APP.summaries = [];
...
APP.build_indices = function () {
  ...
  // hack for static site: remove 'trivial' from the list
  APP.benchmarks = APP.benchmarks.filter(function (d) { return d !== "trivial"; });
